...
\begin{proof}
    ...
    For the step case, suppose that $P(m)$ is true for $m_0 \leq m < n$, we need to prove $P(m)$ is true for $m_0 \leq m < n\next$. Since $P(m)$ holds for $m_0 \leq m < n$ and for $m=n$ (due to the given implication on $P(n)$), $P(m)$ holds for $m_0\leq m \leq n$. The latter term is equivalent to $m_0 \leq m < n\next$ by Proposition~\ref{prop:order_anext_geq_b}. That means $P(m)$ is true for $m_0 \leq m < n\next$. That closes the induction. Now we can conclude that $P(n)$ is true for all natural numbers $n \geq m_0$.
\end{proof}
