...
\end{proposition}
\begin{proof}
    Let's define $Q(n) \iff (m_0 \leq m < n \implies P(m))$. We'll induct on $n$. Although it's not necessary, but I'd like to state the base case explicitly. For the base case $Q(m_0)$, $m_0 \leq m \leq m_0$ and $m\neq m_0$ by Definition~\ref{def:nat_order}. Since $m_0 \leq m$ and $m_0 \geq m$, we can conclude that $m_0\contreq m$ by Proposition~\ref{prop:order_antisymmetric}, what contradicts our antecedent of $m \neq m_0$. Thus, the inductive hypothesis is vacuous, and the implication holds for all cases. 

    For the step case, suppose that $Q(n)$ and $Q(n) \implies P(n)$, then $P(n), \forall m_0\leq m \leq n < n\next$ and $m\neq n$. Which is equivalent to $m_0 \leq n < n\next, P(n)$ what is by definition $Q(n\next)$. Now we have closed the induction by Axiom~\ref{ax:induc}.
\end{proof}