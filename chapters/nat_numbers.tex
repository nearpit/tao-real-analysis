...
\end{proof}
\begin{proposition}
    \textup{(Multiplication preserves the order).} If $a, b$ are natural numbers such that $a < b$, and $c$ is positive, then $ac<bc$.
\end{proposition}
\begin{proof}
    Let's induct on $c$. Since $c$ is positive, we can denote it as $c\next$. The base case: 
    \begin{align*}
        a(c\next) &< b(c\next)\\
        a(0\next) &< b(0\next) \\
        (a\times 0) + a &<(b\times 0) + b && \text{By Definition~\ref{def:nat_mul} and Lemma~\ref{lem:nat_mul_commutative}}\\
        a &< b && \text{By Definitions~\ref{def:nat_mul} and~\ref{def:sum}}
    \end{align*}
    For the step case, given $a(c\next)<b(c\next)$, we need to prove that $a((c\next)\next)<b((c\next) \next)$:
    \begin{align*}
