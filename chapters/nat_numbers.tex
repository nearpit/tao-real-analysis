...
\begin{proof}
    Since $q$ is a positive number, we can denote it as $\bar{q}\next$. Now, we fix $\bar{q}\next$ and induct on $n$. For the base case $n=0$, we have $0 = m(\bar{q}\next) + r$. To satisfy the statement, both terms of RHS must be equal to zero. Indeed, there exist $m=0$ and $r=0$ that satisfy the statement and do not violate the condition $0\leq r < q$. For the step case, we assume that the implication holds for $n$, and we need to prove that it holds for $n\next$. $n$
\end{proof}
