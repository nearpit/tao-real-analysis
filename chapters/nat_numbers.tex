...
\begin{subsequence}
    ...
    \end{proof}
    \begin{lemma}
        \textup{(Multiplication is commutative)}. Let $n, m$ be natural numbers. Then $n\times m = m \times n$.
    \end{lemma}
    \begin{proof}
...
\begin{proof}
    ...
    The right to left case: if either $n$ or $m$ is zero, then $n\times m = 0 \times m := 0$ by Definition~\ref{def:nat_mul} or $n \times m = n \times 0 = 0 \times n = 0$ by Lemma~\ref{lem:nat_mul_0_commutative} and Definition~\ref{def:nat_mul}. What closes both directions. 
    
    If both, $n$ and $m$, are positive natural numbers (i.e.~$n\next$ and $m\next$), then $(n\next)\times(m\next)$ is also positive as shown above.
\end{proof}

\begin{proposition}
     \textup{(Distributive law).} For any natural numbers $a, b, c$, we have $a(b + c) = ab + ac$ and $(b + c)a = ba + ca$.
\end{proposition}
